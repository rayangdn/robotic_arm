...
ser = serial.Serial('/dev/ttyUSB0', 9600, timeout=1)  # Adjust the port accordingly
...
x_init, y_init = forward_kinematics_2_dof(0, 0, l1, l2)

# Create the plot elements
link1, = ax.plot([0, l1 * np.cos(np.deg2rad(0))], [0, l1 * np.sin(np.deg2rad(0))], 'r-', lw=3)
link2, = ax.plot([l1 * np.cos(np.deg2rad(0)), x_init], [l1 * np.sin(np.deg2rad(0)), y_init], 'b-', lw=3)
